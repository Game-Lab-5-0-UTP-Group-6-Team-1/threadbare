...
class_name FillGameLogicelementals 
extends Node
...
func start() -> void:
	var player: Jozu = get_tree().get_first_node_in_group("jozu")
	if player:
		player.mode = Jozu.Mode.FIGHTING
...
func _update_allowed_colors() -> void:
	...
	for enemy: ThrowingEnemyelementals in get_tree().get_nodes_in_group("throwing_enemy"):
...
func _on_barrel_completed() -> void:
	...
	var player: Jozu  = get_tree().get_first_node_in_group("player")
	if player:
		player.mode = Jozu.Mode.COZY
	goal_reached.emit()
